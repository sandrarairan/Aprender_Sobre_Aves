                          ...
                          mainPanel(class="body",width=10,
                                    ...
                                    lapply(1:length(nombre_cientifico),function(i) {
                                      wellPanel(fluidRow(
                                        ...
                                        img(src=specfile[i]),
                                        tags$audio(src=imgfile[i],type = "audio/wav", controls = "false"),
...
server <- function(input, output) {
  ...
  nombrecientifico<-list.files("www/",pattern="(..jpg)") #find audio files; 
  nombre_cientifico<-sapply(nombre_cientifico,function(x) tools::file_path_sans_ext(x),USE.NAMES = F) 
  
  #Sample the files initially (random)
  isolate({
    vals$ordr<-sample(1:length(nombre_cientifico),length(nombre_cientifico),replace=F)
  })
  ...
  observeEvent(input$call,{
    ...
    print("---------------------")
    key<-as.matrix(t(rbind(1:vals$counter,nombre_cientifico[vals$ordr[vals$counter]])))
  ...
  fullQuiz<-reactive({
    ...
    for(i in 1:vals$counter){
      ...
      audio<-if(input$showaudio){tags$audio(src=imgfile[whichSample],type = 'audio/wav', controls = 'false')}else{}
      
      pic<- if(input$showpic){img(src=nombrecientifico[whichSample],height="200px",width="auto")}else{}
      
      spec<- if(input$showspec){img(src=specfile[whichSample])}else{}
      
      video<-if(input$showvideo){tags$video(src=videofile[whichSample],type = 'video/mp4', controls = NA,height="200px",width="auto")}else{}
